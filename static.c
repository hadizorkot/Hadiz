...
int main(int argc, char** argv) {
    ...
    int* local_data = (int*)malloc(sizeof(int) * WIDTH * rows_per_process);
    ...
    if (rank == 0) {
        global_data = (int*)malloc(sizeof(int) * WIDTH * HEIGHT);
    }

    double start_communication_time = MPI_Wtime();
    MPI_Gather(local_data, WIDTH * rows_per_process, MPI_INT, global_data, WIDTH * rows_per_process, MPI_INT, 0, MPI_COMM_WORLD);
    double end_communication_time = MPI_Wtime();
    communication_time = end_communication_time - start_communication_time;

    free(local_data);

    if (rank == 0) {
        FILE* file_ptr = fopen("mandelbrot.pgm", "wb");
        fprintf(file_ptr, "P5\n%d %d\n255\n", WIDTH, HEIGHT);
        for (int i = 0; i < WIDTH * HEIGHT; i++) {
            fputc(global_data[i], file_ptr);
        }
        fclose(file_ptr);
        free(global_data);
    }
